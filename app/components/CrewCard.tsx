  ...
  return (
    <article className={className}>
      {/* .crew-slot (className above) sets pointer-events: none - these
          ...
          background. crew-overlay-card-trigger re-enables pointer-events
          just for this button so the rest of the poster art stays inert,
          and takes over the full slot's position/size so the
          photo/name/role/bio inside keep the exact same absolute-positioned
          layout as before. */}
      <button
        type="button"
        className="crew-overlay-card-trigger"
        onClick={(event) => onSelect?.(member, event.currentTarget)}
        aria-haspopup="dialog"
        aria-label={`View ${member.name}'s biography`}
      >
        <div className="crew-overlay-card-photo">
          ...
          <span
            className="crew-overlay-card-initials"
            aria-hidden="true"
        ...
        <h3 className="crew-overlay-card-name">{member.name}</h3>
        <p className="crew-overlay-card-role">{member.role}</p>
        <p className="crew-overlay-card-bio">
          {member.bio}
          {member.quote ? (
            <span className="crew-overlay-card-quote">&ldquo;{member.quote}&rdquo;</span>
          ) : null}
        </p>
      </button>
    </article>
